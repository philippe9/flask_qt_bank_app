...
from repository.client_repo import create_user, login_user, update_user_solde, get_user, list_all_users
from logger import LOGGER
from models.return_model import ReturnModel
from repository.transaction_repo import create_transaction, list_transactions, list_all_transactions_repo
...
def do_transaction(identifiant: str, pin: str, amount:int, compte_recepteur:str) -> ReturnModel:
    ...
    return ReturnModel(False)
def do_transaction_flask(compte_emmeteur: str, amount:int, compte_recepteur:str) -> ReturnModel:
    user = get_user(compte_emmeteur)
    if(user is not None and user is not False):
        receiver = get_user(compte_recepteur)
        if(receiver is not None and receiver is not False):
            if(user.solde > amount):
...
def list_user_transaction(identifiant: str, pin: str,nombre_trx: int) -> bool |list[Transaction] :
    ...
    return False
def list_all_transactions() -> bool |list[Transaction] :
    return list_all_transactions_repo()
def list_users() -> list[Transaction]:
