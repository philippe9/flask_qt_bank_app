...
.join(Client, onclause=Transaction.compte_emmeteur == Client.num_compte)
    ...
    try:
        ...
        LOGGER.error(f"Unexpected error when creating user: {e}")
        raise e
def list_all_transactions_repo() -> list[Transaction]:
   
    try:
        transactions = session.query(Transaction).join(Client, onclause=or_(Transaction.compte_emmeteur == Client.num_compte, Transaction.compte_recepteur == Client.num_compte)).all()
